...
void loop() {
  ...
  if (S2Val >= pisoPreto && S3Val < pisoPreto && S4Val >= pisoPreto) {
    //Sensor do meio agora esta no branco.
    moveFrente(160);
    ultimoMov = 0;
  } else if (S2Val < pisoPreto && S3Val >= pisoPreto && S4Val >= pisoPreto) {
    //Sensor da esquerda (roxo) está no branco.
    moveDireitaPouco(140);
    ultimoMov = 2;
    //delay(1000);
  } else if (S4Val < pisoPreto && S2Val >= pisoPreto && S3Val >= pisoPreto) {
    //Sensor da direita (branco) está no branco.
    moveEsquerdaPouco(140);
    ultimoMov = 1;
    //delay(1000);
    ...
    ultimoMov = 4;
    //delay(1000);
  } else if (S4Val < pisoPreto || S5Val < pisoPreto && S3Val >= pisoPreto) {
    //Sensor da esquerda (verde) está na no branco.S
    moveEsquerdaPouco(120);
    ultimoMov = 3;
  } else if(ultimoMov = 4 && S1Val >= pisoPreto && S2Val >= pisoPreto && S3Val >= pisoPreto && S4Val >= pisoPreto && S5Val >= pisoPreto ){
    moveEsquerdaPouco(160);
  }else if(ultimoMov = 3 && S1Val >= pisoPreto && S2Val >= pisoPreto && S3Val >= pisoPreto && S4Val >= pisoPreto && S5Val >= pisoPreto ){
